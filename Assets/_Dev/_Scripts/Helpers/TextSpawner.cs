...
public class TextSpawner : StaticInstance<TextSpawner>
{
    ...
    public void SpawnAndFade(string textName, string msg, Vector3 worldPosition)
    {
        var selectedTextData = GetItem(textName);
        if (selectedTextData == null) return;

        ProcessSpawnAndFade(selectedTextData, msg, worldPosition);
    }

    private void ProcessSpawnAndFade(TextData textData, string msg, Vector3 worldPos)
    {
        var textObject = ObjectPooler.Instance.Spawn("Text", parentCanvas.transform);
        ...
        if (textObject.TryGetComponent(out TextMeshProUGUI text))
        {
            ...
            text.DOFade(0, 0.5f).SetEase(Ease.InExpo);
            text.transform.DOMoveY(text.transform.position.y + 100f, 1f).SetEase(Ease.Linear)
                .OnComplete(() => ObjectPooler.Instance.ReleasePooledObject("Text", text.gameObject));
        }
    }
